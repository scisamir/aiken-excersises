...
import { builtinByteString, conStr, NativeScript, UTxO } from "@meshsdk/common";
...
const parameterizedScriptSinglesig = applyParamsToScript(
    smartWalletCode,
    [conStr(0, [builtinByteString(wallet1VK)])],
    "JSON"
);

const scriptAddrSingleSig = serializePlutusScript(
    { code: parameterizedScriptSinglesig, version: 'V3' },
    undefined,
    0
).address;

const parameterizedScriptMultisig = applyParamsToScript(
    smartWalletCode,
    [conStr(1, [builtinByteString(multisigHash)])],
    "JSON"
);

const scriptAddrMultisig = serializePlutusScript(
    { code: parameterizedScriptMultisig, version: 'V3' },
    undefined,
    0
).address;
...
export {
    blockchainProvider,
    txBuilder,
    parameterizedScriptSinglesig,
    parameterizedScriptMultisig,
    scriptAddrSingleSig,
    scriptAddrMultisig,
    wallet1,
    wallet2,
